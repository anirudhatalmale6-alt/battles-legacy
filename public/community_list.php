...
if (!in_array($kind, ['question','recipe','update','healthtip'], true)) $kind = 'update';
...
$META = [
  ...
  'healthtip'=> ['Family Health Tips','What has worked for us &mdash; shared by the family, for the family.',  'Share a Health Tip'],
];
list($ttl, $intro, $cta) = $META[$kind];
page_head($ttl, ['body_class' => 'home fnews']);
?>
<div class="fn-wrap" style="padding-top:22px">
  <p style="margin:0 0 6px"><a class="btn" href="<?= $kind === 'healthtip' ? 'health.php#familytips' : 'news.php' ?>">&larr; Back to <?= $kind === 'healthtip' ? 'Health' : 'Family News' ?></a></p>
    ...
    <div class="cl-grid">
      <?php foreach ($items as $it): ?>
        <div class="fn-col cl-item">
          ...
          <?php if (($kind === 'update' || $kind === 'healthtip') && $it['photo']): ?><div class="cl-photo" style="background-image:url('<?= e($it['photo']) ?>')"></div><?php endif; ?>
          <?php if ($kind === 'recipe'): ?>
          ...
          <?php else: /* update or health tip */ ?>
            <?php if (trim($it['title']) !== ''): ?><h3 class="cl-title"><?= e($it['title']) ?></h3><?php endif; ?>
            <p class="cl-body"><?= nl2br(e(mb_strimwidth($it['body'],0,260,'…'))) ?></p>
            <div class="fn-by"><?= e($it['author']) ?> &middot; <?= e(comm_ago($it['created_at'])) ?></div>
            <a class="btn2 solid" href="community_view.php?id=<?= (int)$it['id'] ?>">Read it all</a>
          <?php endif; ?>
